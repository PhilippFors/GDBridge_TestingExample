...
func _ready() -> void:
	...
	$PortalTraveller.onPortalEnter.connect(_on_portal_enter)
	$PortalTraveller.onPortalExit.connect(_on_portal_exit)

func _physics_process(delta) -> void:
	if !is_multiplayer_authority():
		return
		
	var Move = $"../Move Functions"
	var input = $"../Input"
	...
	if is_on_floor():
		if input.jump_on:
			...
			Move._airaccelerate(delta, input.move_dir.normalized(), input.move_dir.length(), Parameters.AIR_ACCELERATION)
		else:
			Move._friction(delta, 1.0)
			Move._accelerate(delta, input.move_dir.normalized(), input.move_dir.length(), Parameters.ACCELERATION)
	else: 
		Move._airaccelerate(delta, input.move_dir.normalized(), input.move_dir.length(), Parameters.AIR_ACCELERATION)
...
func _on_portal_enter():
	pass
	#print("I entered a portal")
	
func _on_portal_exit():
	pass
	#print("I exited a portal")
