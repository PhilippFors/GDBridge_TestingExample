...
@export var camera : Camera3D ## Camera node that is automatically rotated to compensate for the camera anchor rotation.
@export var viewmodelcamera : Camera3D ## viewmodel cam idk
@export var portalTraveller : PortalTraveller
...
func _ready() -> void:
	...
	camera.current = true
	viewmodelcamera.current = true
		
	#$"Arm/Arm Anchor/MainCamera/SubViewportContainer/SubViewport/ViewmodelCam".current = true
...
func _should_interpolate() -> bool:
	...
	return (Engine.get_frames_per_second() > Engine.physics_ticks_per_second || (Engine.physics_ticks_per_second % roundi(Engine.get_frames_per_second())) != 0) && !portalTraveller.isTravelling
