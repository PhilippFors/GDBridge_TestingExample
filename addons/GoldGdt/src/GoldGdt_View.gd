...
@export var input: GoldGdt_Controls
...
func _handle_camera_input(lookInput: Vector2) -> void:
	horizontal_view.rotate_object_local(Vector3.DOWN, lookInput.x)
	horizontal_view.orthonormalize()
	
	vertical_view.rotate_object_local(Vector3.LEFT, lookInput.y)
...
func _calc_roll(rollangle: float, rollspeed: float) -> float:
	...
	var side = Body.bodyVelocity.dot(horizontal_view.transform.basis.x)
