...
func _update_portal_camera_near_clip_plane(camera):
	...
	#camera.near = abs(other_portal.global_position - camera.global_position).length() - NEAR_CLIP_OFFSET
	# Oblique plane when very close to portal causes glitching/visual artifacts, so only enable if a small distance away
		...
		#camera.Projection = (Projection(Vector4.ZERO, Vector4.ZERO, Vector4.ZERO, Vector4.ZERO))
